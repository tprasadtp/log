...
// This is adapted from hashicorp's hclog-go package.
...
// Analyzer to check for odd ags
var Analyzer = &analysis.Analyzer{
	Name:     "logvet",
	Doc:      "check tprasadtp/log invocations",
	Requires: []*analysis.Analyzer{inspect.Analyzer},
	Run:      run,
}

// We ignore f functions for now as they are not handled
...
func run(pass *analysis.Pass) (interface{}, error) {
	for _, f := range pass.Files {
		ast.Inspect(f, func(n ast.Node) bool {
			...
			if !isNamedType(recv.Type, "github.com/tprasadtp/log", "Logger") {
				return true
			}
